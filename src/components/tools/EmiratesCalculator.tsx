...
export default function EmiratesCalculator() {
  ...
  const calculate = () => {
    ...
    let note =
      "Under 20 employees — not currently subject to mandatory quotas. Participation in Nafis is still commercially useful.";
    ...
    if (hc >= 50) {
      ...
      breakdown = gap
        ? [
            ...
            {
              label: "2026 rate (projected)",
              amount: gap * 9000 * 12,
              sublabel: "AED 9,000 / month per unfilled position",
            },
          ]
        : [];
      note =
        "Companies with 50+ mainland employees must reach 8% Emiratisation of skilled roles in 2025, rising to 10% by 2026. The monthly contribution is AED 8,000 per unfilled position in 2025 — up from AED 7,000 in 2024 and increasing to AED 9,000 in 2026.";
      ...
      breakdown = gap
        ? [
            {
              label: "2024 contribution",
              amount: c2024,
              sublabel: "AED 96,000 per unfilled position · collected Jan 2025",
            },
            {
              label: "2025 contribution",
              amount: c2025,
              sublabel: "AED 108,000 per unfilled position · collected Jan 2026",
            },
          ]
        : [];
      note =
        "Mainland companies with 20–49 employees in MoHRE-targeted sectors must hire 2 Emiratis per year. MoHRE refers to non-compliance amounts as annual financial contributions, not fines. The 2024 contribution of AED 96,000 per unfilled position was collected in January 2025; the 2025 contribution rises to AED 108,000 per unfilled position, collectible in January 2026.";
    } else if (hc >= 20) {
      note =
        "Your sector is not currently in the targeted 20–49 employee categories. Scope is expanding, so proactive Nafis planning is recommended.";
    }
    ...
    <section className="max-w-3xl">
      ...
      <div className="border border-ink/10 bg-clay/35 p-6">
        <div className="grid gap-5 md:grid-cols-2">
          <div>
            <FieldLabel>Total mainland employees</FieldLabel>
            <ToolInput
              type="number"
              min={1}
              value={headcount}
          ...
          <div>
            <FieldLabel>Current Emirati employees</FieldLabel>
            <ToolInput
              type="number"
              min={0}
              value={current}
          ...
          <div>
            ...
            <ToolSelect value={sector} onChange={(e) => setSector(e.target.value)}>
              ...
              {TARGETED_SECTORS.map((s) => (
                <option key={s} value="targeted">
                  {s}
                </option>
              ))}
            </ToolSelect>
        ...
        <button
          onClick={calculate}
          className="mt-6 bg-sienna px-7 py-4 font-dm text-xs font-bold uppercase tracking-wider2 text-paper transition-colors hover:bg-umber"
        >
        ...
        <OutputBox label="Your Emiratisation picture — 2025">
          <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-4">
            {[
              ...
              [
                result.exposure ? aed(result.exposure) : "AED 0",
                result.exposureLabel,
                result.exposure ? "text-risk-red" : "text-ink",
              ],
              [
                result.status,
                "Compliance status",
          ...
          {result.breakdown.length > 0 && (
            <div className="mb-5 border border-ink/10 bg-paper p-5">
              <p
                className="mb-3 font-dm text-[11px] font-bold uppercase text-ink/55"
          ...
          <p
            className="mt-5 border-t border-ink/10 pt-4 font-dm text-xs leading-6 text-ink/45"
            style={{ fontWeight: 300 }}
          >
            <span className="font-bold uppercase tracking-wider text-ink/55" style={{ letterSpacing: "0.08em" }}>
              Disclaimer ·{" "}
            </span>
            Figures based on MoHRE Cabinet Decision (Ministerial Decision No. 455 of 2023). Last verified: April 2026.
            This calculator provides indicative figures only and is not legal advice. Consult a qualified UAE employment
            lawyer for your specific obligations.
          </p>
          <UpsellStrip
            title="Want a full compliance plan?"
            body="I’ll review your setup, calculate Nafis subsidy angles, and give you a 90-day plan."
            href="/business#services"
            link="See Emiratisation Pack →"
          />
          <UpsellStrip
      ...
      <SuggestionBox
        title="Need a different compliance calculation?"
        subtitle="Saudisation, Omanisation, or a different regulatory question? Tell me what you need."
        placeholder="e.g. I need a Saudisation calculator for my KSA office..."
      />
    </section>
  );
}
