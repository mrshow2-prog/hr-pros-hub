...
const CV_TYPES: { id: CVType; title: string; desc: string }[] = [
  { id: "chronological", title: "Chronological", desc: "A clear timeline of roles. Best when your trajectory tells the story." },
  { id: "skills", title: "Skills-led", desc: "Capabilities first, history second. Good for pivots or non-linear paths." },
  { id: "hybrid", title: "Hybrid", desc: "A short skills summary followed by full experience. The most versatile." },
];
...
export function isIntentReady(f: ReturnType<typeof useCVBuilder>["state"]["intentForm"]) {
...
export default function IntentFields() {
  ...
  const addRole = (raw: string) => {
    ...
    if (!v || f.targetRoles.includes(v) || f.targetRoles.length >= MAX_ROLES) return;
    patchIntent({ targetRoles: [...f.targetRoles, v] });
    setRoleInput("");
  };
  const removeRole = (r: string) => patchIntent({ targetRoles: f.targetRoles.filter((x) => x !== r) });
  ...
  const toggleAgnostic = () => {
    ...
    patchIntent({ industryAgnostic: next, targetIndustry: next ? null : "" });
  };
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-ink/15 bg-paper px-3 py-2 focus-within:border-sienna">
          {f.targetRoles.map((r) => (
            <span key={r} className="inline-flex items-center gap-1.5 rounded-full bg-sienna/10 px-3 py-1 font-dm text-sm text-sienna">
              {r}
              <button type="button" onClick={() => removeRole(r)} aria-label={`Remove ${r}`} className="text-sienna/70 hover:text-sienna">
                <X size={12} />
              </button>
            </span>
          ))}
          {f.targetRoles.length < MAX_ROLES && (
            <input
              ...
              placeholder={f.targetRoles.length === 0 ? "e.g. HR Director — press Enter to add" : "Add another role"}
      ...
      <div>
        ...
        <p className="mb-3 font-dm text-xs text-ink/45">The professional discipline your role sits in</p>
          ...
          {FUNCTIONS.map((fn) => (
            <button
              ...
              className={cn(
                ...
                f.functionArea === fn ? "border-sienna bg-sienna text-paper" : "border-ink/20 bg-paper text-ink/70 hover:border-ink/40",
              )}
            >
              {fn}
            </button>
      ...
      <div>
        ...
        <label className="mt-3 inline-flex cursor-pointer items-center gap-2 font-dm text-sm text-ink/70">
          <input type="checkbox" checked={f.industryAgnostic} onChange={toggleAgnostic} className="h-4 w-4 accent-sienna" />
        ...
        <div className="flex flex-wrap gap-2">
          {SENIORITY.map((s) => (
            <button
              ...
              className={cn(
                "rounded-full border px-4 py-2 font-dm text-sm transition-colors",
                f.seniority === s.id ? "border-sienna bg-sienna text-paper" : "border-ink/20 bg-paper text-ink/70 hover:border-ink/40",
              )}
            >
              {s.label}
            </button>
          ...
          {CV_TYPES.map((t) => (
            <button
              ...
              className={cn(
                ...
                f.cvType === t.id ? "border-sienna bg-clay/40" : "border-ink/15 bg-paper hover:border-ink/30",
              )}
            >
        ...
        <div className="inline-flex rounded-md border border-ink/15 bg-paper p-1">
          {TONES.map((t) => (
            ...
            >
              ...
              <span className={cn("block text-[10px]", f.tone === t.id ? "text-paper/75" : "text-ink/45")}>{t.hint}</span>
            </button>
          ))}
        </div>
      </div>
