...
export default function TemplateCasablanca({
  ...
  const mainRenderers: Partial<Record<SectionKey, () => JSX.Element>> = {
    ...
    certifications: () => (
      <section className="mb-7">
        <MainH icon={BadgeCheck}>Certifications</MainH>
          ...
          {cv.certifications.map((c) => (
            <div key={c.id} className="flex items-baseline justify-between gap-3 text-[12.5px] text-ink/85">
              <div>
                ...
                {c.issuer && (
                  <span style={{ color: accentBg as string }}> — {c.issuer}</span>
                )}
              </div>
  ...
  return (
    <div className="mx-auto flex w-[794px] max-w-full bg-white text-ink font-dm text-left">
      ...
      >
        ...
        {photoUrl && shouldRender(cv, "contact") && (
        ...
        {shouldRender(cv, "contact") && contactRows.length > 0 && (
          <section className="mb-7">
