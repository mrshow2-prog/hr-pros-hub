...
/**
 * Tokyo — Bold gradient header band with overlapping circular photo and a
 * clean, modern body. Section headers carry small coloured icon chips.
 * Inspired by Novoresume / Resume.io premium picks.
 */
  ...
  return (
    <Page>
      ...
      <header
        ...
        style={{
          background: `linear-gradient(125deg, ${accent} 0%, rgba(0,0,0,0.55) 130%)`,
        }}
      >
        ...
        />
        <span
          className="absolute -bottom-8 right-24 h-20 w-20 rotate-45 border-2 border-paper/15"
          aria-hidden
        />
        ...
        {isVisible(cv, "contact") && contactItems.length > 0 && (
          <div className="relative mt-6 flex flex-wrap gap-2">
            {contactItems.map(([Icon, v], i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1.5 rounded-full bg-paper/15 px-3 py-1 text-[11.5px] text-paper backdrop-blur-sm"
              >
              ...
              {cv.experience.map((exp) => (
                <article key={exp.id} className="grid grid-cols-[110px_1fr] gap-5">
                  <div className="pt-1 text-right">
                    <p
                      className="font-dm text-[11px] font-bold uppercase tracking-[0.1em]"
                  ...
                  <div className="relative border-l-2 pl-5" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
                    <span
                      className="absolute -left-[7px] top-1.5 h-3 w-3 rounded-full ring-2 ring-paper"
                      style={{ background: accent as string }}
                    />
                    ...
                    {visibleBullets(exp).length > 0 && (
                      <ul className="space-y-1.5">
                        {visibleBullets(exp).map((b) => (
                          ...
                          >
                            <span
                              className="absolute left-0 top-[8px] h-1 w-2 rounded-sm"
                              style={{ background: accent as string }}
                            />
              ...
              <div className="space-y-3">
                {cv.education.map((ed) => (
                  <div key={ed.id}>
                    ...
                    <p className="text-[12px] font-semibold" style={{ color: accent as string }}>
                      {ed.institution}
                    </p>
                    {ed.period && (
                      <p className="text-[11px] text-ink/55">{ed.period}</p>
                    )}
                  </div>
                ))}
              </div>
            ...
            <section>
              ...
              <ul className="space-y-1.5">
                {cv.achievements.map((a, i) => (
                  <li
                    key={i}
                    className="relative pl-4 text-[12.5px] leading-[1.55] text-ink/85"
                  >
                    <span
