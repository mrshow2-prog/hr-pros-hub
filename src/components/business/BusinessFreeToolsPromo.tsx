    ...
    <section id="free-tools-promo" className="px-6 md:px-20 py-20 bg-cream-warm">
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          <div>
            ...
            <p className="font-dm max-w-lg text-ink/55" style={{ fontWeight: 300, fontSize: "1rem" }}>
              Generate a JD, write a policy, calculate your Emiratisation fine exposure, or run a full HR health diagnostic — all free, no email required.
            </p>
          </div>
          <Link
            to="/tools"
            className="font-dm font-bold text-sm uppercase px-8 py-4 rounded-sm whitespace-nowrap transition-colors duration-200 bg-terracotta hover:bg-terracotta-deep text-cream self-start md:self-auto"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {FREE_TOOLS.map((t, i) => (
            ...
            >
              ...
              <h3 className="font-dm font-bold text-sm mb-2 text-ink">{t.name}</h3>
        ...
        <div className="mt-10 p-6 flex flex-col md:flex-row items-center justify-between gap-5 bg-ink">
          <div>
            <p className="font-serif font-semibold text-lg mb-1 text-cream">Liked what you saw?</p>
            <p className="font-dm text-sm text-cream/50" style={{ fontWeight: 300 }}>
              Every free tool is a small window into how we think. A 30-minute call goes much further.
            </p>
          </div>
          <a
            href="#contact"
            className="font-dm font-bold text-xs uppercase px-7 py-3.5 rounded-sm whitespace-nowrap transition-colors duration-200 bg-terracotta hover:bg-terracotta-deep text-cream"
